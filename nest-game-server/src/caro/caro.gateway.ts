import {
  ...
  import { CaroService, Cell } from './caro.service';
  ...
  export class CaroGateway implements OnGatewayConnection, OnGatewayDisconnect {
    ...
    async handleConnection(client: Socket) {
      try {
        ...
        if (!decoded || !decoded.sub) return client.disconnect();
      ...
      for (const [roomId, room] of this.rooms) {
        ...
        if (idx !== -1) {
          room.players.forEach((p) => {
            if (p.userId !== userId) p.socket.emit('opponentLeft');
          });
    ...
    async handleQueue(@ConnectedSocket() client: Socket) {
      const userId = (client as any).userId;
      if (!userId) return;
    ...
    async handleMove(
      ...
      @MessageBody() payload: { r: number; c: number },
    ) {
      const userId = (client as any).userId;
      if (!userId) return;
